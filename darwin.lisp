...
(defun close-device (device)
  ...
  (when (effect device)
    (ff-release-effect (dev device) (effect device))
    ...
    (when effect
      (with-device-failures (device)
        (cffi:with-foreign-objects ((definition '(:struct ff-effect))
          ...
          (setf (ff-effect-size effect) (cffi:foreign-type-size '(:struct ff-effect)))
          (setf (ff-effect-flags effect) :cartesian)
          (setf (ff-effect-gain effect) (floor (* 10000 strength)))
          (setf (ff-effect-axe-count effect) 1)
          (setf (ff-effect-axe-identifiers effect) axes)
          (setf (ff-effect-axe-directions effect) direction)
